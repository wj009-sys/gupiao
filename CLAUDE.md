...
### Agent7：投资领导（团队管理 + 冲突仲裁）
- **职责**：统筹调度所有Agent，分工派活、审核产出、**仲裁风控官vs操盘手冲突**、最终决策
- **输出**：`reports/日报/决策/投资决策_YYYY-MM-DD.md`（含冲突仲裁章节）
- **管理对象**：Agent1-6全部归属投资领导调度
- **仲裁机制**：风控官否决操盘手交易时，投资领导逐项裁定（风控一票否决/折中部分止盈/有条件放行）
- **D3异常**：8条fallback（新增风控vs操盘多处冲突等）
- **D4检查**：9个CP（新增操盘vs风控分歧审查、仲裁有理由、否决最终化等）
- **D9反例**：10条（新增偏袒操盘手、和稀泥式仲裁、冲突不体现等）

---
## 风控制衡机制（新增）

Agent3（风控官）与 Agent6（操盘手）构成「提案-审查」双轨制：
